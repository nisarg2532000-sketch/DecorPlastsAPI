    ...
    public class V1Controller : Controller
    {
        ...
        public IActionResult AddCode([FromBody] AddCode AddCode)
        {
            try
            {
                ...
                if (issucess)
                {
                    SpResultcode Codes = _otpBAL.AddCode(AddCode);
                    return Ok(Codes);
                }
                return BadRequest(new { Message = "Token not verified" });
            }
        ...
        [HttpPost("GetWishListOrder")]
        public IActionResult GetWishListOrder([FromBody] getdata getdata)
        {
            try
            {
                bool issucess = _otpBAL.Verifytoken(getdata.userid, getdata.token);
                if (issucess)
                {
                    var getOrderList = _otpBAL.GetWishListOrder(Convert.ToInt32(getdata.userid));
                    return Ok(getOrderList);
                }
                return BadRequest(new { Message = "Token not verified" });
            }
        ...
        [HttpDelete("DeleteFutureOrder")]
        public IActionResult DeleteFutureOrder([FromBody] DeleteOrder deleteOrder)
        {
            try
            {
                bool issucess = _otpBAL.Verifytoken(deleteOrder.userid, deleteOrder.token);
                if (issucess)
                {
                    ...
                    var results = _otpBAL.DeleteFutureOrder(deleteOrder);
                    return Ok(results);
